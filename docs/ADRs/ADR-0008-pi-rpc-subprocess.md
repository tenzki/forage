...
Settings persist typed agent definitions (instructions, model override, tool allowlist) and skill definitions (slash label, instructions, assigned agent). Context follows one application rule rather than a skill setting: the full ancestor path and the invocation's parent branch with all descendants are local input, with the invocation subtree and unrelated higher-level sibling branches excluded; stable internal links in the command add explicitly referenced branches. A top-level command has no automatic local input. Local and referenced sections retain hierarchy, deduplicate overlap, and must fit the fixed 100-node/40,000-character safety budget. Missing references and overflow block generation before an output placeholder is inserted. Global tool enablement is intersected with the selected agent's allowlist before each run.
...
- `src/agent/definitions.ts` defines and validates persisted agents and slash-command skills; obsolete persisted context settings are discarded during loading.
- `src/agent/context.ts` resolves the local branch and stable-ID references in document order, preserves indentation, deduplicates overlap, and enforces the fixed safety budget.
- `src/editor/contextPreview.ts` decorates local nodes, referenced nodes, the excluded invocation, and errors only while the command is focused.
...
- Existing persisted skills load without their obsolete context-strategy field.
- A child command receives its full ancestor path and complete parent branch, including collapsed descendants and excluding its own subtree and unrelated ancestor siblings; a top-level command receives no automatic local branch.
- Stable internal links add referenced branches in appearance order, survive target renames, deduplicate overlap, and block generation when unresolved.
- Local and referenced nodes receive distinct temporary editor decorations while the slash command is composed and focused; those decorations never enter the document, persistence, or undo history.
- Context over 100 nodes or 40,000 serialized characters blocks rather than truncating silently.
