...
If the owner has explicitly published and enabled Inbox automation, an ordered list of rules decides which skills run. Rules are checked top to bottom and only the first enabled matching rule runs its skills. A rule can match sites (a site such as `github.com` also matches `www.github.com` and `gist.github.com`, but not `github.community`), any link, or — for rules published through the API — `source.kind`, exact source properties, and URL type (`youtube`, `x`, or `webpage`). The existing Notes API response is unchanged and returns as soon as the capture and any queued run snapshots commit; transcription or research continues in the background while the desktop is closed. The original shared note is always preserved, including when enrichment later fails.

In Settings → Inbox link rules, add a rule, list its sites, choose one or more published skills, and drag rules by their handle so specific sites sit above a catch-all **Any link** rule. Skills can also be changed on rules published through the API, including the older per-link-type rules; their match stays as published. For genuinely ambiguous captures, an explicitly configured dispatcher agent may choose only from that policy's allowlisted skills; it receives no write-capable tools.

When no rules have been published, Settings starts with disabled GitHub (`github.com`), YouTube (`youtube.com`, `youtu.be`), and X (`x.com`, `twitter.com`) rules. Choose a skill for each rule you keep and enable them only after publishing compatible server skills and enrolling a server credential. A typical YouTube skill requires `youtube_transcript` and asks for a summary plus transcript notes; an X or webpage research skill uses `x_read`, `web_fetch`, and optionally `web_search`. Output is attached below the stable capture and synchronized as ordinary agent-provenance notes.
