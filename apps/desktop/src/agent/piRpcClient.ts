...
export function probePiRuntime(): Promise<PiRuntimeStatus> {
  return probeRuntime('pi-version', 'Pi')
}
...
export class PiRpcClient {
  ...
  async start(options: PiProcessOptions): Promise<void> {
    ...
    const [bridgePath, agentDir] = await Promise.all([
      resolveResource('resources/pi/ai-chat-bridge.ts'),
      appDataDir(),
    ])
    const args = [
      ...PI_ARGS,
    ...
    this.command = Command.create('pi-agent', args, { env })
    this.attachProcessListeners(this.command)
    try {
      this.child = await this.command.spawn()
    } catch (error) {
      throw new Error(`Could not start Pi. Install pi 0.84.2 or later and restart the app. ${message(error)}`)
    }
  }
  ...
  waitForSettled(timeoutMs = 180_000): Promise<void> {
    return new Promise((resolve, reject) => {
      const timer = window.setTimeout(() => {
        unsubscribe()
        reject(new Error(`Timed out waiting for Pi. ${this.stderr}`))
      }, timeoutMs)
      const unsubscribe = this.onEvent((event) => {
        if (event.type === 'process_error') {
          window.clearTimeout(timer)
          unsubscribe()
          reject(new Error(String(event.error || 'Pi subprocess failed.')))
  ...
  async stop(): Promise<void> {
    ...
    if (child) {
      try {
        ...
        console.warn('[pi-rpc] failed to stop subprocess:', error)
      }
    }
    this.rejectPending(new Error('Pi subprocess stopped.'))
  ...
  private attachProcessListeners(command: Command<string>): void {
    ...
    command.on('close', ({ code, signal }) => {
      ...
      this.handleExit(new Error(`Pi exited unexpectedly (code ${code}, signal ${signal}). ${this.stderr}`))
    })
  }
  ...
  private handleStdout(data: string): void {
    ...
    if (this.stdoutBuffer.length > STDOUT_LINE_LIMIT) {
      this.handleExit(new Error('Pi RPC output exceeded the allowed message size.'))
      return
    }
    ...
    for (const line of lines) {
      ...
      try {
        ...
        console.warn('[pi-rpc] ignored malformed output:', error)
      }
    }
  }
  ...
  private route(event: PiRpcEvent): void {
    ...
    if (event.type === 'response' && id && this.pending.has(id)) {
      ...
      if (event.success === false) request.reject(new Error(String(event.error || 'Pi command failed.')))
  ...
  private send(command: Record<string, unknown>): Promise<PiRpcEvent> {
    if (!this.child) return Promise.reject(new Error('Pi subprocess is not running.'))
    const id = `ai-chat-${++this.requestId}`
    return new Promise((resolve, reject) => {
      const timer = window.setTimeout(() => {
        this.pending.delete(id)
        reject(new Error(`Pi did not acknowledge ${String(command.type)}. ${this.stderr}`))
      }, REQUEST_TIMEOUT_MS)
      this.pending.set(id, { resolve, reject, timer })
      void this.child!.write(`${JSON.stringify({ ...command, id })}\n`).catch((error) => {
        window.clearTimeout(timer)
        this.pending.delete(id)
        reject(new Error(`Could not write to Pi: ${message(error)}`))
      })
    })
  }
