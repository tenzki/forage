...
const environmentSchema = z.object({
  ...
  FORAGE_AGENT_ENCRYPTION_KEY: z.string({ error: 'Agent credential encryption key is required; every server process executes agent runs.' }).trim().min(1),
  ...
  FORAGE_OAUTH_VERIFICATION_URL: z.string().url().default('https://auth.openai.com/codex/device'),
  FORAGE_OAUTH_REDIRECT_URI: z.string().url().default('https://auth.openai.com/deviceauth/callback'),
  FORAGE_OAUTH_CLIENT_ID: z.string().trim().min(1).max(500).default('app_EMoamEEZ73f0CkXaXp7hrann'),
...
export interface ServerConfig {
  ...
  agent: {
    ...
    oauth: {
      deviceUrl: string; deviceTokenUrl: string; tokenUrl: string
      verificationUri: string; redirectUri: string; timeoutSeconds: number; clientId: string
    }
  ...
  return {
    ...
    agent: {
      ...
      oauth: {
        ...
        deviceTokenUrl: value.FORAGE_OAUTH_DEVICE_TOKEN_URL,
        tokenUrl: value.FORAGE_OAUTH_TOKEN_URL,
        verificationUri: value.FORAGE_OAUTH_VERIFICATION_URL,
        redirectUri: value.FORAGE_OAUTH_REDIRECT_URI,
        timeoutSeconds: value.FORAGE_OAUTH_TIMEOUT_SECONDS,
        clientId: value.FORAGE_OAUTH_CLIENT_ID,
      },
