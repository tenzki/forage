...
The skill stores the rubric; the invocation supplies the question. Typed text after the command is the question, and the configured question is only a fallback default, so one rubric serves many questions and there is a single text channel to the provider.

The extension returns ordinary text/reference nodes with one link and at most one short value per row: ★ on the selected comparison candidate, the nearest level label for Score, category group bullets for classification, and bare matching links for Noul. Probabilities, scores and confidence go into a plain bullet note on each row (the generic result node's optional `note`), keeping the outline scannable while retaining the numbers. Typed questions already sit in the invocation bullet, so rows go directly under it; a default question becomes the single root that replaces an empty invocation. Do not invent explanations.

Sorting and inclusive Noul filtering use unrounded values; comparison, Score and Noul are highest first, classification follows category order, document order breaks ties and rounding is display-only. Result order and display precision are not configurable. Empty filtering yields a no-matches summary. All these semantics live in the extension.

Tag output is the one opt-in exception to leaving source nodes unchanged. Tags in Forage are inline `#tag` text, so the generic result gains host-applied tag edits (`{ nodeId, add, remove }`) beside `reorder`: the host checks admission, appends missing tags as unmarked text, deletes removed tags with one adjoining space, and applies everything in the run's single undo step. The extension decides the tags: classification maps categories to tags (derived from the label unless set) with an optional minimum probability, and Noul adds one configured tag at the threshold. Each edit removes the rest of the skill's vocabulary so reruns replace earlier answers; a user-typed tag with the same name is indistinguishable and is replaced too. Score and comparison do not label candidates individually, so they have no tag output.
