...
describe('generated image nodes', () => {
  ...
  it('renders and survives JSON reload as an image-only outline node', () => {
    const content = {
      type: 'doc',
      content: [{
        type: 'bulletList',
        content: [{
          ...
          content: [{ type: 'generatedImage', attrs: { src: WEBP, alt: 'Generated visual' } }],
        }],
      }],
    }
    ...
    expect(image?.getAttribute('src')).toBe(WEBP)
